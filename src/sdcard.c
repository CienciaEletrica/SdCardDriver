...
/**
 * @brief Tenta inicializar o cartão até 3 vezes com delay entre tentativas
 *
 * Função interna usada por sd_quick_init e sd_begin.
 * Útil quando o cartão demora para responder (ex: logo após inserção).
 */
...
/**
 * @brief Lê múltiplos blocos sequenciais
 * 
 * @param ctx Ponteiro para o contexto da biblioteca.
...
SD_Status sd_blk_write(SD_Context *ctx, uint32_t block, const uint8_t *buffer)
{
    ...
    ctx->hal.spi_tx_rx(0xFE);                    // Token de início de dados
 ...
 * @param cluster O número do cluster cuja entrada será atualizada.
 * @param value   O valor de 32 bits a ser gravado (próximo cluster ou 0x0FFFFFFF).
 ...
 * @return SD_Status SD_SUCCESS ou SD_ERR_DISK_FULL se não houver espaço.
 */
...
{
    /* 1. Aloca novo cluster usando a função da pág 11 */
    if (sd_fat_alloc_cluster(ctx, fs, new_cluster) != SD_SUCCESS)
        return SD_ERR_DISK_FULL;

    /* 2. Atualiza FAT[current] -> new_cluster (pág 10) */
    ...
    while (cluster < 0x0FFFFFF8 && cluster >= 2) {
        for (uint8_t s = 0; s < fs->sct_per_clus; s++) {
            ...
            for (uint16_t i = 0; i < 512; i += 32) {
                // Se encontrar 0x00, significa que não há mais arquivos NESTE cluster.
                // Mas ainda pode haver arquivos no PRÓXIMO cluster da corrente.
        ...
        // Pula para o próximo cluster da FAT (Pág. 12/21)
...
SD_Status sd_file_read_stream(SD_Context *ctx, FAT32_Info *fs, const SD_FileName *file, sd_stream_callback_t callback)
{
    ...
    while (cluster < 0x0FFFFFF8 && cluster >= 2)
    {
        ...
        // Navega para o proximo cluster usando sua funcao da pag 21
...
SD_Status sd_file_write_stream(SD_Context *ctx, FAT32_Info *fs, const SD_FileName *file, sd_write_callback_t callback)
{
    ...
    // Se o arquivo estiver vazio, não há nada para sobrescrever nesta função (use append)
    if (file_size == 0 || cluster < 2) return SD_SUCCESS;
...
static SD_Status sd_expand_directory(SD_Context *ctx, FAT32_Info *fs, uint32_t start_cluster) {
    ...
    // 2. Alocar novo cluster (Usando sua funcao da pag 11)
    uint32_t new_cluster;
    st = sd_fat_alloc_cluster(ctx, fs, &new_cluster);
    if (st != SD_SUCCESS) return st;

    // 3. Ligar o antigo ao novo (Pag 10)
    ...
    for (uint8_t i = 0; i < fs->sct_per_clus; i++) {
        st = sd_blk_write(ctx, first_sector + i, ctx->buffer); // Funcao da pag 6
        if (st != SD_SUCCESS) return st;
    }
